...
**🔍 Multi-Model Results**
- **Automatic Discovery**: Recursively scans directories for all `.trx` files, no manual configuration
...
- **Offline Capable**: No external dependencies, works without internet connection
...
your_project/
├── bpa_output/
│   ├── SalesModel_20241122_143022.trx    ← Parsed by this tool
│   ├── FinanceModel_20241122_143045.trx  ← Parsed by this tool
│   └── InventoryModel_20241122_143108.trx ← Parsed by this tool
...
- **Severity Badges**: 
  - 🟡 **Info (Severity 1)** (yellow badge) - Best practice recommendations
  - 🟠 **Warning (Severity 2)** (orange badge) - Important but non-critical issues
  - 🔴 **Error (Severity 3)** (red badge) - Critical issues requiring immediate attention
    ...
    steps:
      ...
      - name: Run BPA Analysis
        run: |
          ...
          foreach ($model in $models) {
            $output = "bpa_output/$($model.BaseName)_$(Get-Date -Format 'yyyyMMdd_HHmmss').trx"
      ...
      - name: Fail on Errors
        run: |
          ...
          $errorCount = (Select-String -Path "bpa_output/*.trx" -Pattern 'outcome="Failed"').Count
